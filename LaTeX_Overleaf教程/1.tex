...
6.4.4 模型数学公式与解释现在，我们来详细阐述综合动态路径规划与优化模型的数学公式及其解释。

$\begin{aligned}\text{首先，我们需要定义螺线的参数方程。对于给定的螺距 p}&=1.7\text{ 米，螺线方程}\\\text{可以表示为：}\end{aligned}$
$$\begin{aligned}&r=a\theta\\&x=a\theta\mathrm{cos}\theta\\&y=a\theta\mathrm{sin}\theta\end{aligned}$$
其中，$a=\frac p{2\pi}=\frac{1.7}{2\pi}\approx0.2706$米是螺线参数，$\theta$是极角，$(x,y)$是笛卡尔坐标。
对于盘入螺线，我们可以定义其参数方程为：
$$\begin{array}{c}x_{in}(\theta)=a\theta\mathrm{cos}\theta\\y_{in}(\theta)=a\theta\mathrm{sin}\theta\end{array}$$
对于盘出螺线，考虑到它与盘入螺线中心对称，其参数方程为：
$$x_{out}(\theta)=-a\theta\mathrm{cos}\theta\\y_{out}(\theta)=-a\theta\mathrm{sin}\theta $$
